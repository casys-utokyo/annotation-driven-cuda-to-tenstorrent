  ...
  tileplan.device_for (%arg2, %arg3) in (%c10_0, %c10) {
    ...
    ^bb0(%arg4: memref<5120x5120xbf16>, %arg5: memref<5120x5120xbf16>):
      ...
      tileplan.temporal_for (%arg6) in (0, 5119, 1) {
        %2 = tileflow.transfer %1 core_dims(%arg6, %arg2, %arg3) core_symbols() core_map affine_map<(d0, d1, d2) -> ((d1 floordiv 10) * 10 + (d0 floordiv 32) floordiv 16, (d2 floordiv 10) * 10 + (d0 floordiv 32) floordiv 16)> tile_dims(%arg6) tile_symbols() tile_map affine_map<(d0) -> ((d0 floordiv 32) mod 16, (d0 floordiv 32) mod 16)> core_boundary in_bounds tile_boundary in_bounds : !tileplan.shard_view -> !d2mtile.tile<bf16>
        ...
        tileplan.shard_for (%arg7) in (%c16_1) axis <row> {
          ...
          %14 = tileflow.row_mask symbols(%13) set affine_set<(d0)[s0] : (d0 + s0 >= 0)> invert : !tileflow.mask<bf16>
          tileplan.shard_for (%arg8) in (%c16_3) axis <col> {
            %15 = tileflow.transfer %1 core_dims(%arg3, %arg8, %arg6, %arg2, %arg3) core_symbols() core_map affine_map<(d0, d1, d2, d3, d4) -> ((d3 floordiv 10) * 10 + (d2 floordiv 32) floordiv 16, (d4 floordiv 10) * 10 + d1 floordiv 16 + d0)> tile_dims(%arg3, %arg8, %arg6) tile_symbols() tile_map affine_map<(d0, d1, d2) -> ((d2 floordiv 32) mod 16, d1 mod 16)> core_boundary in_bounds tile_boundary in_bounds : !tileplan.shard_view -> !d2mtile.tile<bf16>
            %16 = tileflow.broadcast %15 kind row from(%8) : !d2mtile.tile<bf16> -> !d2mtile.tile<bf16>
            %17 = tileflow.local_read %1 tile_dims(%arg2, %arg3, %arg7, %arg8) tile_symbols() tile_map affine_map<(d0, d1, d2, d3) -> (d2 mod 16, d3 mod 16)> tile_boundary in_bounds : !tileplan.shard_view -> !d2mtile.tile<bf16>
            ...
            %21 = tileflow.col_mask symbols(%20) set affine_set<(d0)[s0] : (d0 + s0 >= 0)> invert : !tileflow.mask<bf16>
            %22 = tileflow.masked_overlay %19 with %17 active(%14, %21) source_map affine_map<(d0, d1) -> (d0, d1)> symbols() : !d2mtile.tile<bf16>
