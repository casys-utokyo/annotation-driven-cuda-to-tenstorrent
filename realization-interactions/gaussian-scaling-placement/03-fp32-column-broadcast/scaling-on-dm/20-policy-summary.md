# Annotations in `gaussian-fp32/mask-dm-bcast-c-scale`

## What was asked for

| key | first seen | answered by | on | values |
|---|---|---|---|---|
| `tileflow.realization_engine` | `front-2-annotated` | `30-realized-tileflow` -- the policy tail, per op (see Support/RealizationEngine.h) | `d2mtile.binary`, `d2mtile.unary`, `tileflow.broadcast`, `tileflow.mask_fill` | `#tileflow.engine<compute>`, `#tileflow.engine<dm>` |
...
| `tileflow.consumer_bcast` | `30-realized-tileflow` | `0-infer-engines` | (unit) |
