...
# --- the annotated front -------------------------------------------------
BASE=${BASE:-../../../../shared-inputs/jacobi-2d-bf16}
W="$OUT/.authoring"
cp "$BASE/10-tileplan.mlir" "$W"
patch -s -p0 "$W" < 21-schedule-annotations.diff
"$TASK_OPT" "$W" > "$OUT/15-scheduled-tileplan.mlir"
run "$OUT/15-scheduled-tileplan.mlir" "$W" --realize-tileplan-schedule
patch -s -p0 "$W" < 21-policy-annotations.diff
run "$W" "$W.next" --apply-tileflow-policy \
    --apply-tileflow-policy-stop-after=overlay-copies
mv "$W.next" "$W"
patch -s -p0 "$W" < 21e-sites-annotation.diff
