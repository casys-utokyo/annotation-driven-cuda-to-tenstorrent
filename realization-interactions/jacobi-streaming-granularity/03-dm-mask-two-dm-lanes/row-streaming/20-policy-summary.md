# Annotations in `jacobi/unfused-wb-dm-stream-2dm`

## What was asked for

| key | first seen | answered by | on | values |
|---|---|---|---|---|
| `tileflow.realization_engine` | `front-2-semantic` | `30-realized-tileflow (before 23-realized-pins.diff)` -- the policy tail, per op (see Support/RealizationEngine.h) | `tileflow.col_copy`, `tileflow.masked_overlay`, `tileflow.row_copy` | `#tileflow.engine<dm>` |
...
| `tileflow.overlay` | `30-realized-tileflow (before 23-realized-pins.diff)` | (to the end) | `#tileflow.overlay_realization<dm_copy>` |
