...
#map2 = affine_map<()[s0, s1] -> (s0 + (s1 floordiv 10) * 10)>
...
module {
  func.func @_Z11syrk_kerneliiPfS__driver(%arg0: memref<3200x3200xbf16>, %arg1: memref<3200x3200xbf16>) -> memref<3200x3200xbf16> attributes {tt.d2m_emittable = true, tt.d2m_unhandled = 0 : i64, tt.function_type = "forward_device", tt.l1_peak = 1437696 : i64, tt.l1_persistent_base = 1028096 : i64, tt.l1_pool = 1028096 : i64} {
    ...
    d2m.generic {block_factors = [], grid = #ttcore.grid<10x10>, indexing_maps = [], iterator_types = [], threads = [#d2m.thread<datamovement>, #d2m.thread<datamovement>, #d2m.thread<compute>]}
      ...
      %core0 = d2m.core_index(0) : index
      %core1 = d2m.core_index(1) : index
      %1 = affine.apply #map()[%core0]
      scf.for %arg2 = %c0 to %c10 step %c1 {
        %2 = affine.apply #map1(%arg2)[%core1]
        d2m.remote_load %alloc[%1, %2] into %0 mcore[%core0, %c0] mshape[%c1, %c10] srcSlice[%c0, %c0] : memref<10x10x10x10x!ttcore.tile<32x32, bf16>, #ttcore.shard<20480x2048, 1>, #l1> into !d2m.cb<memref<10x10x!ttcore.tile<32x32, bf16>, #l1>>
      ...
      %core0 = d2m.core_index(0) : index
      %core1 = d2m.core_index(1) : index
      %1 = affine.apply #map2()[%core1, %core0]
      scf.for %arg2 = %c0 to %c10 step %c1 {
        %2 = affine.apply #map1(%arg2)[%core1]
        d2m.remote_load %alloc[%1, %2] into %0 mcore[%c0, %core1] mshape[%c10, %c1] srcSlice[%c0, %c0] : memref<10x10x10x10x!ttcore.tile<32x32, bf16>, #ttcore.shard<20480x2048, 1>, #l1> into !d2m.cb<memref<10x10x!ttcore.tile<32x32, bf16>, #l1>>
      } {d2m.blocking_loop = 1 : i64}
    }, {
