...
# --- the loop schedule: this rung asks for none ---------------------------
BASE=${BASE:-../../../../shared-inputs/syrk-bf16}
"$TOTT" --tile-spec="$BASE/01-tiling-spec.yaml" \
        --emit-tileplan-decided="$OUT/.plan" \
        --emit-tileflow="$OUT/.flow" --stop-after-tileflow-plan \
        "$BASE/00-cuda-input.mlir" > /dev/null
"$TASK_OPT" "$OUT/.plan" > "$OUT/15-scheduled-tileplan.mlir"
"$TASK_OPT" "$OUT/.flow" > "$OUT/22-annotated-tileflow.mlir"
rm -f "$OUT/.plan" "$OUT/.flow"

# --- realize the policy ---------------------------------------------------
run "$OUT/22-annotated-tileflow.mlir" "$OUT/30-realized-tileflow.mlir" --realize-tileflow-policy
# Attributes stamped after realization, on ops the policy tail creates.
patch -s -p0 "$OUT/30-realized-tileflow.mlir" < 23-realized-pins.diff
"$TASK_OPT" "$OUT/30-realized-tileflow.mlir" > "$OUT/.pinned" && mv "$OUT/.pinned" "$OUT/30-realized-tileflow.mlir"
...
[ "$OUT" = "." ] && exit 0
bad=0
for f in 15-scheduled-tileplan.mlir \
         22-annotated-tileflow.mlir \
