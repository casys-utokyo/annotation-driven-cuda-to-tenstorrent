  ...
  func.func @_Z11syrk_kerneliiPfS__driver(%arg0: memref<3200x3200xbf16> {tileplan.abi_index = 0 : i64, tileplan.tensor_role = #tileplan.tensor_role<input>}, %arg1: memref<3200x3200xbf16> {tileplan.abi_index = 1 : i64, tileplan.tensor_role = #tileplan.tensor_role<inout>}) attributes {tileplan.selected_compute} {
    ...
    tileplan.device_for (%arg2, %arg3) in (%c10_0, %c10) {
      ...
      ^bb0(%arg4: memref<3200x3200xbf16>, %arg5: memref<3200x3200xbf16>):
        %2 = tileplan.l1_buffer role<receive> elem bf16 tile(32, 32) domain(1, 1) order(0) {task.buffer_depth = 2 : i64} : !tileplan.l1_buffer
        %3 = tileplan.l1_buffer role<receive> elem bf16 tile(32, 32) domain(1, 1) order(1) {task.buffer_depth = 2 : i64} : !tileplan.l1_buffer
        ...
        task.group @dm1_0 engine(<dm1>) domain(<phase>) reads(%0 : !tileplan.l1_buffer [full]) writes(%2 : !tileplan.l1_buffer [full]) {
          ...
          tileplan.shard_for (%arg6) in (%c10_1) axis <row> {
            tileplan.shard_for (%arg7) in (%c10_2) axis <col> {
              tileplan.temporal_for (%arg8) in (0, 100, 1) {
                ...
                tileflow.bulk_transfer %0 into %2 source_core(%4, %5) source_slice(%6, %7) multicast(%c0, %arg3, %c10_3, %c1) epoch(%arg8) {task.dm_lane = #task.engine<dm1>, task.engine_label = "dm"} : !tileplan.l1_buffer into !tileplan.l1_buffer
              } {epoch_id = 0 : i64}
            }
          }
        }
        task.group @dm0_0 engine(<dm0>) domain(<phase>) reads(%0 : !tileplan.l1_buffer [full]) writes(%3 : !tileplan.l1_buffer [full]) {
          %c10_1 = arith.constant 10 : index
          %c10_2 = arith.constant 10 : index
          %c0 = arith.constant 0 : index
          %c1 = arith.constant 1 : index
          ...
          tileplan.shard_for (%arg6) in (%c10_1) axis <row> {
            tileplan.shard_for (%arg7) in (%c10_2) axis <col> {
              tileplan.temporal_for (%arg8) in (0, 100, 1) {
                %4 = affine.apply #map3(%arg2, %arg6)
                %5 = affine.apply #map1(%arg8, %arg3)
                %6 = affine.apply #map2(%arg6)
                %7 = affine.apply #map2(%arg8)
                tileflow.bulk_transfer %0 into %3 source_core(%4, %5) source_slice(%6, %7) multicast(%arg2, %c0, %c1, %c10_3) epoch(%arg8) {task.engine_label = "dm"} : !tileplan.l1_buffer into !tileplan.l1_buffer
              } {epoch_id = 0 : i64}
            }
          }
        }
