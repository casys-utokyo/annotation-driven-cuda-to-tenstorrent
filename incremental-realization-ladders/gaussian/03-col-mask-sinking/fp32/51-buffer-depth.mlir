...
module {
  func.func @_Z10ForwardSubPfS__driver(%arg0: memref<5120x5120xf32> {tileplan.abi_index = 1 : i64, tileplan.tensor_role = #tileplan.tensor_role<inout>}) attributes {tileplan.selected_compute} {
    ...
    tileplan.device_for (%arg1, %arg2) in (%c10_0, %c10) {
      %0 = tileplan.persistent_l1 elem f32 tile(32, 32) domain(16, 16) order(0) {logical_tiles = [160, 160], partition = [10, 10], physical_grid = [10, 10], shard = [16, 16], tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
      tileplan.phase name "compute" ins() outs(%arg0 : memref<5120x5120xf32>) {
      ^bb0(%arg3: memref<5120x5120xf32>):
        %1 = tileplan.l1_buffer role<receive> elem f32 tile(32, 32) domain(1, 1) order(0) {task.delivery_initialized, task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %2 = tileplan.l1_buffer role<constructed> elem f32 tile(32, 32) domain(1, 1) order(1) {task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %3 = tileplan.l1_buffer role<receive> elem f32 tile(32, 32) domain(16, 1) order(2) {frame_axis = #tileplan.axis<row>, task.delivery_initialized, task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %4 = tileplan.l1_buffer role<receive> elem f32 tile(32, 32) domain(1, 16) order(3) {frame_axis = #tileplan.axis<col>, task.delivery_initialized, task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %5 = tileplan.l1_buffer role<constructed> elem f32 tile(32, 32) domain(1, 16) order(4) {frame_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.frame_access = "replay", task.reuse = 16 : i64, task.slot_access = #task.slot_access<replay>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %6 = tileplan.l1_buffer role<constructed> elem f32 tile(32, 32) domain(1, 16) order(5) {frame_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.frame_access = "replay", task.reuse = 16 : i64, task.slot_access = #task.slot_access<replay>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %7 = tileplan.l1_buffer role<constructed> elem f32 tile(32, 32) domain(16, 1) resident(1, 1) order(6) {frame_axis = #tileplan.axis<row>, task.drain_axis = #tileplan.axis<row>, task.fill_axis = #tileplan.axis<row>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<row>, tt.mailbox_depth = 2 : i64} : !tileplan.l1_buffer
        ...
        %9 = tileplan.l1_buffer role<intermediate> name "dst_spill_8" elem f32 tile(32, 32) domain(1, 1) order(8) {tt.dst_spill, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %10 = tileplan.l1_buffer role<intermediate> name "dst_spill_9" elem f32 tile(32, 32) domain(1, 1) order(9) {tt.dst_spill, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        %11 = tileplan.l1_buffer role<intermediate> name "dst_spill_10" elem f32 tile(32, 32) domain(1, 1) order(10) {tt.dst_spill, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
