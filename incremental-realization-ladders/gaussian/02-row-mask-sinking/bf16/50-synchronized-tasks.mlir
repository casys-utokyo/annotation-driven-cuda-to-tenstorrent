  ...
  func.func @_Z10ForwardSubPfS__driver(%arg0: memref<5120x5120xbf16> {tileplan.abi_index = 1 : i64, tileplan.tensor_role = #tileplan.tensor_role<inout>}) attributes {tileplan.selected_compute} {
    ...
    tileplan.device_for (%arg1, %arg2) in (%c10_0, %c10) {
      ...
      ^bb0(%arg3: memref<5120x5120xbf16>):
        ...
        %6 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(1, 16) order(5) {frame_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.frame_access = "replay", task.reuse = 16 : i64, task.slot_access = #task.slot_access<replay>} : !tileplan.l1_buffer
        %7 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(16, 1) resident(1, 1) order(6) {frame_axis = #tileplan.axis<row>, task.drain_axis = #tileplan.axis<row>, task.fill_axis = #tileplan.axis<row>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<row>} : !tileplan.l1_buffer
        %8 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(16, 1) resident(1, 1) order(7) {frame_axis = #tileplan.axis<row>, task.drain_axis = #tileplan.axis<row>, task.fill_axis = #tileplan.axis<row>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<row>} : !tileplan.l1_buffer
        %9 = tileplan.l1_buffer role<intermediate> name "dst_spill_8" elem bf16 tile(32, 32) domain(1, 1) order(8) {tt.dst_spill} : !tileplan.l1_buffer
        %10 = tileplan.l1_buffer role<intermediate> name "dst_spill_9" elem bf16 tile(32, 32) domain(1, 1) order(9) {tt.dst_spill} : !tileplan.l1_buffer
        %11 = tileplan.l1_buffer role<intermediate> name "dst_spill_10" elem bf16 tile(32, 32) domain(1, 1) order(10) {tt.dst_spill} : !tileplan.l1_buffer
        ...
        tileplan.temporal_for (%arg4) in (0, 5119, 1) {
          task.group @dm0_0 engine(<dm0>) domain(<phase>) reads() writes() {
            ...
            tileplan.shard_for (%arg5) in (%c16_1) axis <col> {
              ...
              %27 = tileflow.col_mask symbols(%26) set #set invert {task.engine_label = "dm"} : !tileflow.mask<bf16>
          ...
          task.group @compute_1 engine(<compute>) domain(<phase>) reads() writes() {
            ...
            tileplan.shard_for (%arg5) in (%c16) axis <row> {
              ...
              tileplan.shard_for (%arg6) in (%c16_1) axis <col> {
                %27 = tileflow.read_l1_buffer %5 at(%c0, %arg6) epoch(%arg4) : !tileplan.l1_buffer -> !d2mtile.tile<bf16>
                %28 = affine.apply #map7(%arg1, %arg2, %arg5, %arg6)
                %29 = affine.apply #map8(%arg1, %arg2, %arg5, %arg6)
                %30 = d2mtile.dst_acquire capacity 8 : !d2mtile.dst_bank
                %31 = tileflow.read_l1_buffer %0 at(%28, %29) epoch(%arg4) {tt.dst_slot = 0 : i64} : !tileplan.l1_buffer -> !d2mtile.tile<bf16>
                %c0_17 = arith.constant 0 : index
                %c0_18 = arith.constant 0 : index
                %32 = tileflow.read_l1_buffer %11 at(%c0_17, %c0_18) epoch(%arg4) : !tileplan.l1_buffer -> !d2mtile.tile<bf16>
                %33 = d2mtile.binary tile_mul %32, %27 -> "source_ssa" {task.engine_label = "compute", tt.dst_slot = 1 : i64} : !d2mtile.tile<bf16>
                %34 = d2mtile.binary tile_sub %31, %33 -> "source_ssa" {task.engine_label = "compute", tt.dst_slot = 1 : i64} : !d2mtile.tile<bf16>
                %35 = tileflow.read_l1_buffer %6 at(%c0_2, %arg6) epoch(%arg4) {tt.dst_slot = 2 : i64} : !tileplan.l1_buffer -> !tileflow.mask<bf16>
                %36 = tileflow.compute_select %34 with %31 active(%35 : !tileflow.mask<bf16>) {task.engine_label = "compute", tileflow.overlay = #tileflow.overlay_realization<compute_select>, tt.dst_slot = 1 : i64} : !d2mtile.tile<bf16>
                tileflow.write_l1_buffer %0 at(%arg5, %arg6) epoch(%arg4) to %36 {task.engine_label = "compute"} : !tileplan.l1_buffer to !d2mtile.tile<bf16>
                d2mtile.dst_release %30 : !d2mtile.dst_bank
              }
