        ...
        tileplan.temporal_for (%arg4) in (0, 5119, 1) {
          task.group @dm0_0 engine(<dm0>) domain(<phase>) reads() writes() {
            ...
            tileplan.shard_for (%arg5) in (%c16_1) axis <col> {
              ...
              %27 = tileflow.col_mask symbols(%26) set #set invert {task.engine_label = "dm"} : !tileflow.mask<f32>
          ...
          task.group @compute_1 engine(<compute>) domain(<phase>) reads() writes() {
            ...
            tileplan.shard_for (%arg5) in (%c16) axis <row> {
              ...
              tileplan.shard_for (%arg6) in (%c16_1) axis <col> {
                ...
                %32 = tileflow.read_l1_buffer %11 at(%c0_17, %c0_18) epoch(%arg4) {tt.dst_slot = 2 : i64} : !tileplan.l1_buffer -> !d2mtile.tile<f32>
                %33 = d2mtile.binary tile_mul %32, %28 -> "source_ssa" {task.engine_label = "compute", tt.dst_slot = 2 : i64} : !d2mtile.tile<f32>
                %34 = d2mtile.binary tile_sub %31, %33 -> "source_ssa" {task.engine_label = "compute", tt.dst_slot = 2 : i64} : !d2mtile.tile<f32>
                %35 = tileflow.read_l1_buffer %6 at(%c0_2, %arg6) epoch(%arg4) {tt.dst_slot = 0 : i64} : !tileplan.l1_buffer -> !tileflow.mask<f32>
                %36 = tileflow.compute_select %34 with %31 active(%35 : !tileflow.mask<f32>) {task.engine_label = "compute", tileflow.overlay = #tileflow.overlay_realization<compute_select>, tt.dst_slot = 2 : i64} : !d2mtile.tile<f32>
                tileflow.write_l1_buffer %0 at(%arg5, %arg6) epoch(%arg4) to %36 {task.engine_label = "compute"} : !tileplan.l1_buffer to !d2mtile.tile<f32>
