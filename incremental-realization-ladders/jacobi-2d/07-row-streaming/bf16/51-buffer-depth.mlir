  ...
  func.func @_Z15runJacobi2DCUDAiiPfS__driver(%arg0: memref<3200x3200xbf16> {tileplan.abi_index = 0 : i64, tileplan.tensor_role = #tileplan.tensor_role<inout>}) attributes {tileplan.selected_compute} {
    ...
    tileplan.device_for (%arg1, %arg2) in (%c10_0, %c10) {
      ...
      ^bb0(%arg3: memref<3200x3200xbf16>):
        ...
        %3 = tileplan.l1_buffer role<receive> elem bf16 tile(32, 32) domain(1, 10) order(1) {frame_axis = #tileplan.axis<col>, task.delivery_initialized, task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        ...
        %5 = tileplan.l1_buffer role<receive> elem bf16 tile(32, 32) domain(10, 1) order(3) {frame_axis = #tileplan.axis<row>, task.delivery_initialized, task.slot_access = #task.slot_access<resident>, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
        ...
        %7 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(10, 10) resident(1, 10) order(5) {frame_axis = #tileplan.axis<row_col>, task.drain_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<row>, tt.mailbox_depth = 2 : i64} : !tileplan.l1_buffer
        %8 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(10, 10) resident(1, 10) order(6) {frame_axis = #tileplan.axis<row_col>, task.drain_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<row>, tt.mailbox_depth = 2 : i64} : !tileplan.l1_buffer
        ...
        %11 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(10, 10) resident(1, 1) order(9) {frame_axis = #tileplan.axis<row_col>, task.drain_axis = #tileplan.axis<col>, task.slot_access = #task.slot_access<single_pass>, tt.mailbox_depth = 2 : i64} : !tileplan.l1_buffer
        %12 = tileplan.l1_buffer role<intermediate> name "dst_spill_10" elem bf16 tile(32, 32) domain(1, 1) order(10) {tt.dst_spill, tt.mailbox_depth = 1 : i64} : !tileplan.l1_buffer
