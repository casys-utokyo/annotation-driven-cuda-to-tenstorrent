  ...
  func.func @_Z15runJacobi2DCUDAiiPfS__driver(%arg0: memref<3200x3200xbf16> {tileplan.abi_index = 0 : i64, tileplan.tensor_role = #tileplan.tensor_role<inout>}) attributes {tileplan.selected_compute} {
    ...
    tileplan.device_for (%arg1, %arg2) in (%c10_0, %c10) {
      ...
      ^bb0(%arg3: memref<3200x3200xbf16>):
        ...
        %16 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(10, 10) resident(1, 1) order(15) {frame_axis = #tileplan.axis<row_col>, task.drain_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<col>} : !tileplan.l1_buffer
        %17 = tileplan.l1_buffer role<constructed> elem bf16 tile(32, 32) domain(10, 10) resident(1, 1) order(16) {frame_axis = #tileplan.axis<row_col>, task.drain_axis = #tileplan.axis<col>, task.fill_axis = #tileplan.axis<col>, task.slot_access = #task.slot_access<single_pass>, task.stream = #tileplan.axis<col>} : !tileplan.l1_buffer
        %18 = tileplan.l1_buffer role<intermediate> name "dst_spill_17" elem bf16 tile(32, 32) domain(1, 1) order(17) {tt.dst_spill} : !tileplan.l1_buffer
